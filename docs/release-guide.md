...
Este documento estabelece o procedimento padrão para realizar o deploy e a distribuição de uma nova versão do **lazy CSS**. O processo é automatizado pelo GitHub Actions, mas exige consistência no versionamento local.
...
* **PATCH (`1.4.X`):** Correções de bugs ou ajustes internos que não mudam como o usuário usa a biblioteca. *Exemplo: Corrigir um comportamento visual de um botão no Safari ou arrumar um espaçamento no `.lazy-stack`.*
* **MINOR (`1.X.0`):** Adição de novas funcionalidades, novas classes ou novos componentes sem quebrar o código antigo. *Exemplo: Criar uma classe `.lazy-badge` ou adicionar suporte a um novo layout na pasta `src/layouts/`.*
* **MAJOR (`X.0.0`):** Mudanças estruturais críticas que quebram a compatibilidade com projetos antigos. *Exemplo: Alterar o prefixo de `lazy-` para `v-`, ou remover uma classe que era largamente utilizada.*
...
```text
  ...
  │  • Roda npm run build     │                 │ • Injeta lazy.min.css     │
...
2. **Aba Releases:** Verifique se uma nova Release com o nome da sua tag foi criada no menu lateral direito do GitHub. Certifique-se de que o arquivo `lazy.min.css` está anexado na seção de *Assets* para download direto.
...
```text
https://cdn.jsdelivr.net/gh/seu-usuario/seu-repositorio@v1.0.1/dist/lazy.min.css
